...
class ChunkModel(BaseDataModel):
    ...
    @classmethod #static method - declare object and call the init collection
    ...
    async def insert_many_chunks(self, chunks: list[DataChunk], batch_size: int = 100):
        ...
        return total_inserted #return the total number of chunks inserted
    
    async def delete_chunks_by_project_id(self, project_id: ObjectId):
        result = await self.collection.delete_many({"chunk_project_id": project_id})
        return result.deleted_count #return the number of chunks deleted
